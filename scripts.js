...
function guessNumber() {
  ...
  if (currentNumber === userNumber) {
    tipElement.innerText = `Acertou! O numero é ${currentNumber}`;
    gameOver();
  } else {
    if (guessesRemaining > 0) {
      tipElement.innerText = `O numero é ${
        currentNumber < userNumber ? "menor" : "maior"
      }`;
    } else {
      tipElement.innerText = `Acabaram suas tentativas! O numero era ${currentNumber}`;
      gameOver();
    }
  }
}
